...
import { shouldThrottle, CONSTANTS as YMCONSTANTS } from '../pubmaticUtils.js';
...
export const setMetaData = (data) => { metaData = { dynamicTimeout: data} };
...
export function processBidRequest(reqBidsConfigObj) {
  ...
  const skipRate = (timeoutConfig?.config?.skipRate !== undefined && timeoutConfig?.config?.skipRate !== null) ? timeoutConfig?.config?.skipRate : YMCONSTANTS.DEFAULT_SKIP_RATE;
  if (shouldThrottle(skipRate)) {
    logInfo(`${CONSTANTS.LOG_PRE_FIX} Dynamic timeout is skipped (skipRate: ${skipRate}%)`);
    setMetaData({skipped: YMCONSTANTS.YM_SKIPPED_INFO.MODULE_SKIPPED});
    return reqBidsConfigObj;
  }

  logInfo(`${CONSTANTS.LOG_PRE_FIX} Dynamic timeout is applying...`);
  setMetaData({skipped: YMCONSTANTS.YM_SKIPPED_INFO.MODULE_APPLIED});
