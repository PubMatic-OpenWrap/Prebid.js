...
describe('S2S Adapter', function () {
  ...
  describe('request function', function () {
    ...
    xit('adds site if app is not present', function () {
      const _config = {
        s2sConfig: CONFIG,
        site: {
          publisher: {
    ...
    xit('always add ext.prebid.targeting.includebidderkeys: true for ORTB', function () {
      const s2sConfig = Object.assign({}, CONFIG, {
        adapterOptions: {
          appnexus: {
            key: 'value'
    ...
    xit('adds s2sConfig ext.prebid.bidderparams to request for ORTB', function () {
      const s2sConfig = Object.assign({}, CONFIG, {
        extPrebid: {
          bidderparams: {
            pubmatic2: {}
    ...
    xit('passes first party data in request', () => {
      const s2sBidRequest = utils.deepClone(REQUEST);
      const bidRequests = utils.deepClone(BID_REQUESTS);
  ...
  describe('response handler', function () {
    ...
    xit('Add new parameters like mi, serverSideResponseTime, originalcpm & originalCurrency to bidobject', function () {
      config.setConfig({ s2sConfig: CONFIG });
      adapter.callBids(REQUEST, BID_REQUESTS, addBidResponse, done, ajax);
    ...
    xit('Add new parameteras prebidBidId to bidobject if present in response', function () {
      config.setConfig({ s2sConfig: CONFIG });
      adapter.callBids(REQUEST, BID_REQUESTS, addBidResponse, done, ajax);
      server.requests[0].respond(200, {}, JSON.stringify(RESPONSE_OPENRTB));
  ...
  describe('s2sConfig', function () {
    ...
    xit('should correctly set the floorMin key when multiple bids with various bidfloors exist', function () {
      const s2sConfig = Object.assign({}, CONFIG, {
        extPrebid: {
          floors: {
            enabled: true
    ...
    xit('should correctly set the floorMin key when multiple bids with various bidfloors exist and ortb2Imp contains the lowest floorMin', function () {
      const s2sConfig = Object.assign({}, CONFIG, {
        extPrebid: {
          floors: {
            enabled: true
