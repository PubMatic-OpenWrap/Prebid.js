 ...
 */
export function getGlobalPbObject() {
  const pbNameSpace = conf[CONSTANTS.CONFIG.COMMON][CONSTANTS.CONFIG.PB_GLOBAL_VAR_NAMESPACE] ||
 ...
 */
export function getGlobalOwObject() {
  const owNameSpace = conf[CONSTANTS.CONFIG.COMMON][CONSTANTS.CONFIG.OW_GLOBAL_VAR_NAMESPACE] ||
 ...
 */
export function shouldThrottle(allowTrafficRate, maxRandomValue) {
 ...
 */
export function getGeoInfo(readFrom, callback) {
  const PREFIX = 'UINFO'; // Prefix used for storing and retrieving geo information in local storage
  const LOCATION_INFO_VALIDITY = 172800000; // Validity period for stored geo information (2 days in milliseconds)

  // Construct the URL for the geo-detection service with the publisher ID from the configuration
  const geoDetectionURL = 'https://ut.pubmatic.com/geo?pubid=' +
    conf[CONSTANTS.CONFIG.COMMON][CONSTANTS.CONFIG.PUBLISHER_ID];

  // Attempt to retrieve geo information from local storage
  const info = getGlobalPbObject().getDataFromLocalStorage(PREFIX, LOCATION_INFO_VALIDITY);
