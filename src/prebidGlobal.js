...
if (window.$$PREBID_GLOBAL$$) { 
  console.warn(`Namespace clash happened, with name: ${'window.$$PREBID_GLOBAL$$'}, now you can provide your custom namespace, by creating new profile version in the UI. 
    Existing PWT version details: ${(window?.PWT?.versionDetails === 'function') ? JSON.stringify(window?.PWT?.versionDetails()) : 'Version details not available'}`); 
}
/* eslint-disable */
